...
class Broker:
    ...
    def publicar(self, mensagem):
        ...
        for seguidor in self.seguidores:
            with Proxy(seguidor["uri"]) as seguidor_proxy:
                epoca , offset = seguidor_proxy.avisa_publicacao()
                return self.log.consulta_publicacao_confirmada(epoca, offset)
    ...
    def confirmar_recebimento(self, epoca , offset_antigo):
        ...
        for i in range(offset_antigo + 1, self.log.consultar_offset(epoca)):
            ...
            if( self.qtd_confirmados[epoca][offset_antigo + i] == self.num_seguidores/2 + 1):
                self.log.confirmar_publicacao(epoca, offset_antigo + i)
    ...
    def avisa_publicacao(self):
        ...
        return self.epoca, self.log.consultar_offset(self.epoca) 
...
class Log:
    ...
    def confirmar_publicacao(self, epoca, offset):
        # irá colocar como 1 a confirmação do publicação na matriz de publicações confirmadas
        if epoca not in self.matriz_publicacoes_confirmadas.columns:
            self.matriz_publicacoes_confirmadas[epoca] = pd.Series(dtype='int')
        self.matriz_publicacoes_confirmadas.at[len(self.matriz_publicacoes_confirmadas), epoca] = 1
    def consulta_publicacao_confirmada(self, epoca, offset):
        if epoca not in self.matriz_publicacoes_confirmadas.columns:
            return 0
        return self.matriz_publicacoes_confirmadas[epoca][offset]



if __name__ == "__main__":
    # Altere o estado para "Lider", "Seguidor" ou "Observador" conforme necessário
    broker = Broker("Lider", "Lider-Epoca-1")
    broker.iniciar()
